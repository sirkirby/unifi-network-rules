...
async def async_save_template(hass: HomeAssistant, coordinators: dict, call: ServiceCall) -> None:
    ...
    LOGGER.debug("Save template service: parsed rule_id %s -> %s (type: %s)", original_rule_id, rule_id, rule_type)
...
async def async_setup_template_services(hass: HomeAssistant, coordinators: dict) -> None:
    ...
    # Register services
    hass.services.async_register(DOMAIN, SERVICE_APPLY_TEMPLATE, handle_apply_template, schema=APPLY_TEMPLATE_SCHEMA)

    hass.services.async_register(DOMAIN, SERVICE_SAVE_TEMPLATE, handle_save_template, schema=SAVE_TEMPLATE_SCHEMA)
