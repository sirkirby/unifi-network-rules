                        ...
                        for rule in rules:
                            ...
                            if rule_unique_id == unique_id:
                                try:
                                    ...
                                    LOGGER.error("Failed to update entity %s rule: %s", entity_id, err)
            ...
            for rule in rules:
                ...
                if rule_name and name_filter in rule_name.lower():
                    ...
                    try:
                        ...
                        LOGGER.error("Failed to update rule %s: %s", rule_name, err)
...
async def async_setup_rule_services(hass: HomeAssistant, coordinators: dict) -> None:
    ...
    # Register services
    hass.services.async_register(DOMAIN, SERVICE_TOGGLE_RULE, handle_toggle_rule, schema=TOGGLE_RULE_SCHEMA)

    hass.services.async_register(DOMAIN, SERVICE_DELETE_RULE, handle_delete_rule, schema=DELETE_RULE_SCHEMA)

    hass.services.async_register(DOMAIN, SERVICE_BULK_UPDATE, handle_bulk_update_rules, schema=BULK_UPDATE_RULES_SCHEMA)
