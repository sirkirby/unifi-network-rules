        ...
        if is_our_entity(entity, DOMAIN):
            ...
            try:
                ...
                if (
                    ...
                    or hass.states.get(entity_id).state == "unavailable"
                ):
                    # Remove entity from registry
                    LOGGER.info("Removing entity %s (unique_id: %s) from registry", entity_id, unique_id)
...
async def async_setup_cleanup_services(hass: HomeAssistant, coordinators: dict) -> None:
    ...
    # Register services
    hass.services.async_register(DOMAIN, SERVICE_FORCE_CLEANUP, handle_force_cleanup, schema=vol.Schema({}))
