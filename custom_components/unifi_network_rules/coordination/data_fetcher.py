...
class CoordinatorDataFetcher:
    ...
    async def fetch_all_entity_data(self) -> dict[str, list[Any]]:
        ...
        rules_data: dict[str, list[Any]] = {entity_type: [] for entity_type in self.entity_type_methods.keys()}
        rules_data.update(
            {
                "firewall_zones": [],
                "devices": [],
    ...
    async def _fetch_standard_entities_parallel(self, rules_data: dict[str, list[Any]]) -> None:
        ...
        for entity_type, api_method in self.entity_type_methods.items():
            tasks[entity_type] = asyncio.create_task(self._fetch_entity_type_safe(entity_type, api_method))
        ...
        try:
            ...
            if led_capable_devices:
                ...
                for device in led_capable_devices:
                    ...
                    LOGGER.info("  %s (%s): LED=%s, Model=%s", device_name, device_mac, led_state, device_model)
    ...
    async def _retry_with_path_fix(
        self, entity_type: str, api_method: Callable, original_error: Exception
    ) -> list[Any]:
        ...
        if hasattr(self.api, "_ensure_proxy_prefix_in_path"):
            try:
                ...
                if retry_result:
                    LOGGER.info("Successfully retrieved %d %s after path fix", len(retry_result), entity_type)
                    return retry_result
            ...
            for rule in qos_rules:
                ...
                if rule_name:
                    ...
                    if normalized_rule_name in oon_policy_names:
                        removed_count += 1
                        LOGGER.info(
                            "Filtering OON policy duplicate from QoS rules: '%s' (matches OON policy name)", rule_name
                        )
                        continue
            ...
            for route in traffic_routes:
                ...
                if route_name:
                    ...
                    if normalized_route_name in oon_policy_names:
                        removed_count += 1
                        LOGGER.info(
                            "Filtering OON policy duplicate from Traffic Routes: '%s' (matches OON policy name)",
                            route_name,
                        )
                        continue
