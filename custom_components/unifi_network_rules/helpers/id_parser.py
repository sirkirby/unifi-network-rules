...
def parse_rule_id(rule_id: str, rule_type: str | None = None) -> tuple[str, str | None]:
    ...
    if "_" in rule_id and rule_id.startswith("unr_"):
        ...
        if len(parts) >= 3 and parts[0] == "unr":
            ...
            if not detected_rule_type:
                ...
                LOGGER.debug(
                    "[ID_PARSER] Auto-detected rule_type: %s (from hint: %s)", detected_rule_type, extracted_type_hint
                )
