...
class ApiOperationQueue:
    ...
    def __init__(
        self, delay_between_requests: float = 0.5, max_queue_size: int = 1000, operation_timeout: float = 300.0
    ):
    ...
    async def add_operation(
        self, operation_func: Callable, *args, is_priority: bool = False, max_retries: int = 3, **kwargs
    ) -> asyncio.Future:
        ...
        try:
            ...
            LOGGER.debug(
                "API operation added to %s queue: %s (max_retries=%d)",
            ...
            try:
                ...
                try:
                    ...
                    if not self.priority_queue.empty():
                        queued_item = await asyncio.wait_for(self.priority_queue.get(), timeout=0.1)
                        # Use a shorter delay for priority operations
                        current_delay = max(0.2, self.delay / 2)
                        queue_type = "priority"
                    else:
                        # Then check regular queue with timeout
                        queued_item = await asyncio.wait_for(self.regular_queue.get(), timeout=1.0)
            ...
            try:
                ...
                for expired_op in expired_operations:
                    ...
                    if not expired_op.future.done():
                        ...
                        LOGGER.error("Dropping expired operation %s: %s", expired_op.operation_func.__name__, error_msg)
    ...
    async def _execute_operation(self, queued_op: QueuedOperation, queue_type: str):
        ...
        LOGGER.debug(
            f"Processing API operation from {queue_type} queue: {queued_op.operation_func.__name__} "
        ...
        try:
            ...
            if queued_op.retry_count > 0:
                LOGGER.info(
                    "Operation %s succeeded after %d retries", queued_op.operation_func.__name__, queued_op.retry_count
                )
            ...
            if is_retryable and queued_op.can_retry():
                # Add to retry queue
                LOGGER.warning(
                    "Operation %s failed (attempt %d/%d), will retry: %s",
                ...
                if queued_op.retry_count >= queued_op.max_retries:
                    ...
                    LOGGER.error(
                        "Operation %s failed with non-retryable error: %s", queued_op.operation_func.__name__, error_msg
                    )
