...
class AuthenticationMixin:
    ...
    async def async_init(self, hass=None):
        ...
        try:
            ...
            LOGGER.debug("Created Configuration object with host %s and port %d", base_host, port)
    ...
    async def _try_login(self) -> bool:
        ...
        if hasattr(self, "_last_successful_login"):
            if (
                current_time - self._last_successful_login < self._min_login_interval
                and self._last_successful_login > 0
            ):
                LOGGER.debug(
        ...
        async with self._login_lock:
            ...
            try:
                ...
                if status_code == 429 or "limit_reached" in error_str:
                    ...
                    raise CannotConnect(f"Login rate limited (429): {err}") from err
    ...
    async def refresh_session(self, force: bool = False) -> bool:
        ...
        LOGGER.debug("Current path config before refresh - base: %s, site: %s", original_base_path, original_site_path)
        ...
        try:
            ...
            has_session = (
                hasattr(self.controller, "session")
                and getattr(self.controller, "session", None) is not None
                and getattr(self.controller, "headers", {}).get("Cookie") is not None
            )
            ...
            if resp.status == 200:
                ...
                LOGGER.warning("Session refresh failed (status %d), falling back to full login", resp.status)
                login_success = await self._try_login()
