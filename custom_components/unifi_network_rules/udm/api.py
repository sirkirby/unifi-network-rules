...
from .profiles import PortProfilesMixin, RadiusProfilesMixin, WanSlaProfilesMixin, WlanRateProfilesMixin
...
class UDMAPI(
    ...
    async def handle_auth_failure(self, error_message=None):
        ...
        try:
            ...
            if self._consecutive_auth_failures >= self._max_auth_failures:
                LOGGER.error(
                    "Maximum authentication failures reached (%d). Implementing backoff before next attempt.",
                    self._consecutive_auth_failures,
                )
    ...
    async def queue_api_operation(
        self, operation_func: Callable[..., Awaitable[Any]], *args, **kwargs
    ) -> Awaitable[Any]:
    ...
    def create_api_request(self, method: str, path: str, data: Any = None, is_v2: bool = False) -> Any:
        ...
        """
        LOGGER.debug("Creating API request: method=%s, endpoint=%s, is_v2=%s", method, path, is_v2)
