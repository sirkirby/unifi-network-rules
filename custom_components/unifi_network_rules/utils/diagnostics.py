...
def analyze_controller(controller: Any) -> dict[str, Any]:
    ...
    result = {"class": controller.__class__.__name__, "connectivity": {"is_unifi_os": None}, "capabilities": []}
    ...
    if hasattr(controller, "is_unifi_os"):
        result["connectivity"]["is_unifi_os"] = controller.is_unifi_os
...
def log_controller_diagnostics(controller: Any, api_instance: Any = None) -> None:
    ...
    try:
        ...
        if api_instance:
            ...
            has_session = hasattr(api_instance, "_session") and api_instance._session is not None
