...
async def async_setup_entry(
    ...
    from .network import UnifiNetworkSwitch, UnifiWlanSwitch
    from .oon_policy import UnifiOONPolicyKillSwitch, UnifiOONPolicySwitch
    ...
    from .traffic_route import UnifiTrafficRouteKillSwitch, UnifiTrafficRouteSwitch, UnifiTrafficRuleSwitch
    ...
    LOGGER.debug(
        "Using known_unique_ids populated during initialization: %d entries", len(coordinator.known_unique_ids)
    )
            ...
            try:
                ...
                if rule_type == "traffic_routes" and "kill_switch_enabled" in rule.raw:
                    ...
                    if kill_switch_id not in potential_entities_data:
                        ...
                        LOGGER.debug(
                            "Gathered potential kill switch entity: %s (for parent %s)", kill_switch_id, rule_id
                        )
    ...
    if hasattr(coordinator, "devices") and coordinator.devices:
        ...
        for device in coordinator.devices:
            ...
            if unique_id not in potential_entities_data:
                ...
                LOGGER.info(
                    "Gathered potential LED switch: %s for device %s", unique_id, getattr(device, "name", device.mac)
                )
    else:
        LOGGER.warning(
            "No LED-capable devices found in coordinator. Coordinator devices: %s",
    ...
    for unique_id, data in potential_entities_data.items():
        try:
            ...
            entity = entity_class(coordinator, data["rule_data"], data["rule_type"], config_entry.entry_id)

            # Check if the created entity's unique_id matches the key (sanity check)
            if entity.unique_id != unique_id:
                LOGGER.error(
                    "Mismatch! Expected unique_id %s but created entity has %s. Skipping.", unique_id, entity.unique_id
                )
                continue
