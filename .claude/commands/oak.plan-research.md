...
**HOW TO LAUNCH:** Use the `Task` tool with `run_in_background: true`. Launch ALL parallel tasks in a single message with multiple Task tool calls.
...
- **Use Claude's web tools or keyless MCP web-search (DuckDuckGo/fetch)**: Agent-specific research guidance is incorporated above