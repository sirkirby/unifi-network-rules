...
async def test_init_creates_controller():
    ...
    with (
        ...
        patch(
            "custom_components.unifi_network_rules.udm.authentication.AuthenticationMixin._try_login", return_value=True
        ) as mock_login,
        patch("custom_components.unifi_network_rules.udm.api.UDMAPI.refresh_all") as mock_refresh,
    ):
