...
**HOW TO LAUNCH:** Use Mission Control to assign multiple tasks in parallel. Each task runs in its own cloud environment. Monitor progress and guide agents with real-time steering.
...
- **Use Copilot's built-in web tools + keyless MCP**: Agent-specific research guidance is incorporated above